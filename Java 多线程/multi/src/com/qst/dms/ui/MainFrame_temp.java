...
public class MainFrame_temp extends JFrame {
	...
	private JMenuItem miGather, miSave, miSend,
			miShow, miExit, miCheck, miAbout,miMactchLog,miMatchTrans;
	...
	private void initMenu() {
		...
		miMactchLog = new JMenuItem("匹配日志数据");
		miMactchLog.addActionListener(new MatchLogListener());
		miMatchTrans = new JMenuItem("匹配物流数据");
		miMatchTrans.addActionListener(new MatchTransListener());
		menuMatch.add(miMactchLog);
	...
	private void initTransGatherGUI() {
		...
		cmbTanStatus.setEditable(false);
	...
	private class SaveDataListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			JOptionPane.showMessageDialog(null, "数据保存成功！", "提示",
					JOptionPane.INFORMATION_MESSAGE);*/
		}
	}
		...
		public void actionPerformed(ActionEvent e) {
			try {
				...
				if(matchedTrans != null && matchedTrans.size() > 0) {
					...
					ObjectOutputStream logOutputStream = new ObjectOutputStream(transSocket.getOutputStream());
